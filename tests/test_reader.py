        ...
        for halfring in [0, 1, 2]:
            for bp_corr in [False]:
                if (bp_corr and halfring!=0):
                    ...
                    try:
                        read_map(freq, surv, chtag='', halfring=halfring, pol="I", bp_corr=bp_corr, baseline_length="1s")
    ...
    for surv in ["nominal", "full"] + list(range(1, 5+1)):
        for halfring in [0, 1, 2]:
            if isinstance(surv, int) and halfring != 0:
                ...
                try:
                    read_map(freq, surv, chtag=chtag, halfring=halfring, pol="I", baseline_length="1s")
                except exceptions.IOError as e:
                    pass
