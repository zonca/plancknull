...
import os
from plancknull.reader import SingleFolderToastReader
...
lfi_folder = "/global/project/projectdirs/planck/user/zonca/issues/dx9_toast/maps"

read_map = SingleFolderToastReader(lfi_folder)
        ...
        for halfring in [0, 1, 2]:
            for bp_corr in [False, True]:
                if (isinstance(surv, int) and halfring != 0) or (bp_corr and halfring!=0):
                    ...
                    try:
                        read_map(freq, surv, chtag='', halfring=halfring, pol="I", bp_corr=bp_corr)
                    except exceptions.IOError as e:
                        pass

freq = 70
for chtag in ["18_23", "19_22", "20_21"]:
    for surv in ["nominal", "year1", "year2"]:
        for halfring in [0, 1, 2]:
